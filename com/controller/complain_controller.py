...
def admin_load_complain_reply():
    try:
        if admin_login_session() == 'admin':
            ...
            return render_template("admin/addComplainReply.html", complain_vo_list=complain_vo_list)
        else:
            return admin_logout_session()
    except Exception as ex:
...
@app.route("/user/load_complain")  # user side load the page of complain with previous complain
def user_load_complain():
    try:
        if admin_login_session() == 'user':
            return redirect("/user/view_complain")  # redirect from user_view_complain through fetch the data
        else:
            return admin_logout_session()
    except Exception as ex:
        print("in user_load_complain route exception occured>>>>>>>>>>", ex)
        ...
        if admin_login_session() == 'user':
            ...
            if len(complain_vo_list) != 0:
                ...
                if len(complain_vo_updated_list) == 0:
                    ...
                    if complain_vo_updated_list[0][1].complain_to_login_id is not None:
                        print("complain_vo_updated_list[0][1].complain_to_login_id", complain_vo_updated_list)
                        admin_login_id = complain_vo_updated_list[0][1].complain_to_login_id
